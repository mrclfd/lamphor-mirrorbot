    ...
    if len(args) > 1:
        ...
        with download_dict_lock:
            ...
            try:
                ...
                pass
    if len(args) == 1:
        msg = f"Please reply to the <code>/{BotCommands.MirrorCommand.replace("(", "").replace(")", "").replace("'", "").replace(",", "").replace(" ", "").replace("seed", "")}</code> message which was used to start the download or send <code>/{BotCommands.CancelMirror} GID</code> to cancel it!"
